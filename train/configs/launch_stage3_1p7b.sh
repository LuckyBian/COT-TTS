...
INIT_CKPT="${INIT_CKPT:-}"
if [[ -z "$INIT_CKPT" ]]; then
  echo "ERROR: please provide INIT_CKPT for stage3_1p7b." >&2
  exit 1
fi
...
OUT_DIR="${OUT_DIR:-$PROJECT_ROOT/outputs/stage3_1p7b/${RUN_ID}}"
mkdir -p "$OUT_DIR"
LOG_PATH="$OUT_DIR/train.log"

bash train.sh tasks/train_torch.py configs/stage3_1p7b.yaml \
  ...
  --train.wandb_name qwen3-stage3-1p7b-1ep \
  2>&1 | tee "$LOG_PATH"
