...
INIT_CKPT="${INIT_CKPT:-}"
if [[ -z "$INIT_CKPT" ]]; then
  echo "ERROR: please provide INIT_CKPT for stage2_0p6b." >&2
  exit 1
fi
...
OUT_DIR="${OUT_DIR:-$PROJECT_ROOT/outputs/stage2_0p6b/${RUN_ID}}"
mkdir -p "$OUT_DIR"
LOG_PATH="$OUT_DIR/train.log"

bash train.sh tasks/train_torch.py configs/stage2_0p6b.yaml \
