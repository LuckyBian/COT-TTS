...
DEFAULT_OUTPUT_PATH = PROJECT_ROOT / "data" / "asr.train.jsonl"
DEFAULT_TASK_NAME = "ASR"
...
def build_user_content(task_name: str, target_audio: str) -> str:
    return (
        f"<bos>"
        ...
        f"<audio_ref_start><audio_ref_end>"
        f"<target_end>"
        f"<output_start>"
    )


def build_assistant_content(text: str) -> str:
    return (
...
def build_record(
    ...
    pt_path = codec_dir / f"{utt_id}.pt"
    pt = load_codec_pt(pt_path)
    if pt is None:
        return None

    target_audio = target_audio_tokens_from_pt(pt, global_prefix, semantic_prefix, global_prefix_len)
    if not target_audio:
        return None
    ...
    return {
        ...
        "source_name": "asr",
        "messages": [
            {"role": "user", "content": build_user_content(task_name, target_audio), "loss_mask": 0},
            {"role": "assistant", "content": build_assistant_content(text), "loss_mask": 1},
        ],
        "meta": {
            "task": task_name,
            "wav_path": wav_path,
            "codec_pt_path": str(pt_path),
            "text": text,
        },
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build ASR training jsonl from TSV(path,text) and BiCodec .pt files.")
    parser.add_argument("--tsv", required=True, help="Input TSV with at least `path` and `text` columns.")
